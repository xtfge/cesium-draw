...
<template>
  <div id="markerContainer">
    ...
    <contextMenuViewer :viewer="viewer" ref="contextMenu" :contextMenuItems="contextMenu"></contextMenuViewer>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      _this.billboards = this.viewer.scene.primitives.add(new Cesium.BillboardCollection())
      _this.labels = this.viewer.scene.primitives.add(new Cesium.LabelCollection())
    ...
    methods: {
      ...
      init(viewer,mode) {
        ...
        _this.handler.setInputAction((evt) => {
          ...
          if (Cesium.defined(objs)) {
            for (let obj in objs) {
              if (objs[obj].primitive instanceof Cesium.Billboard&&/^marker.*/.test(objs[obj].id)) {
